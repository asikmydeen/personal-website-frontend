  ...
  return (
    <div className={`relative flex flex-col ${className}`}>
      {label && (
        <motion.label
          htmlFor={inputId}
          className={`absolute left-3 pointer-events-none origin-left ${labelClassName}`}
          initial="rest"
          animate={animateLabel ? getLabelState() : undefined}
      ...
      <motion.input
        ref={inputRef}
        id={inputId}
        className={`px-3 py-2 border rounded-md bg-transparent ${inputClassName}`}
        initial="rest"
        animate={animateFocus ? getInputState() : undefined}
        variants={animateFocus ? inputVariants : undefined}
        onFocus={handleFocus}
        onBlur={handleBlur}
