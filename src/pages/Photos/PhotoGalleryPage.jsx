...
const PhotoGalleryPage = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        // Optionally, load some initial photos not tied to an album or recent photos
        // const photosResponse = await searchPhotos({ recent: true }); // Example
        // if (photosResponse.success) setPhotos(photosResponse.data.photos || []);
  ...
  const handlePhotoUploaded = (newPhoto) => {
    ...
    // For simplicity, if an album is selected, add there, otherwise to general photos
    if (selectedAlbum && newPhoto.albumId === selectedAlbum) {
        setPhotos(prev => [newPhoto, ...prev]);
    } else if (!selectedAlbum) {
        // Potentially add to a general photo list if that's a feature
        // Or refetch albums if new photo created a new album implicitly
    }
    // Could also refetch albums if a new photo might create a new album or update counts
    listAlbums().then(res => res.success && setAlbums(res.data.albums || []));
  };
        ...
        <div className="md:col-span-1 bg-gray-50 p-4 rounded-lg shadow">
          ...
          {albums.length > 0 ? (
            <ul className="space-y-2">
              <li 
                key="all-photos"
                onClick={() => { setSelectedAlbum(null); listAlbums().then(r => r.success && setPhotos([])); /* Clear photos or load all */ }}
                className={`p-2 rounded-md cursor-pointer ${!selectedAlbum ? 'bg-indigo-500 text-white' : 'bg-white hover:bg-indigo-100'}`}
              >
                All Photos (or clear selection)
              </li>
              {albums.map(album => (
                <li 
                  key={album.id}
